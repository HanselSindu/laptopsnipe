...
class mcdm_ftopsis_dummy extends CI_Model { //Dummy Version
		...
		while($i<sizeof($idprods)){ 
			...
			while($j<sizeof($criteria)){
				foreach ($sqli as $row){
					...
					if($j==2){$score[$k]=$row->durability; $k++;}
				}
	...
	function getCrispScores($idprod){ 
		...
		$array_crisp = array('ram', 'video_mem', 'storage_capacity', 'weight', 'price');
		$product = array();
		$score = array();
		$i = 0; $j = 0; $k = 0;
		...
		while($i<sizeof($idprod)){
			...
			while($j<sizeof($array_crisp)){
				foreach($sqli as $row){
					...
					if($j==1){$score[0]=$row->weight;$score[1]=$row->weight;$score[2]=$row->weight;} //negative
				}
			$product[$i][$j] = $score;
			$j++; $k = 0;
			}
	...
	function combineFuzzyCrisp($array_fuzzy, $array_crisp){
		...
		$array_fuzzy_n = array('cpu_performance', 'gpu_performance', 'durability');
		$array_crisp_n = array('ram', 'weight');

		$combined_array_n = array_merge($array_fuzzy_n, $array_crisp_n);
		$i=0; $j=0; 
		$criteria_number = sizeof($array_fuzzy[0])/sizeof($array_fuzzy[0][0]); //bug-fix
		
		while($i<sizeof($array_fuzzy)){ //4 jmlh idprods
			while($j<sizeof($combined_array_n)){ //5
				if($j>=$criteria_number){
					$combined_array[$i][$j] = $array_crisp[$i][$j-$criteria_number];
				}	
				else{
	...
	function normalizeFDM($fdm){
		...
		$types = array('positive','positive','positive','positive','negative');
	...
	function presetFuzzyWeight($usage, $price){
		...
		while($j<sizeof($criteria)){
			...
			$sqli= $sql->result();
					...
					if($j==13){$weight[$index]=$price;} //ditentukan oleh user
					}	
					$index=0;
	...
	function weightFDM($n_fdm, $weight){
		$w_fdm = array(); 
		for($i=0;$i<sizeof($n_fdm);$i++){ //4
			for($j=0;$j<sizeof($n_fdm[0]);$j++){ //5
				for($k=0;$k<sizeof($n_fdm[0][0]);$k++){ //3
	...
	function getCriteriaTypes(){
		...
		$array_fuzzy_n = array('cpu_performance', 'gpu_performance', 'battery_life', 'audio_quality', 'keyboard_quality',
			'wifi', 'durability', 'warranty', 'cooling_systems');
		$array_crisp_n = array('ram', 'video_mem', 'storage_capacity', 'weight', 'price');
...
//New FPIS and FNIS Ideal Solution
	// function getFIS($type){
	// 	$fpis_pos = array(1,1,1);
	// 	$fnis_pos = array(0,0,0);
	// 	$fpis_neg = array(1,1,1);
	// 	$fnis_neg = array(0,0,0);

	// 	if($type='positive'){
	// 		$fis=array($fpis_pos, $fnis_pos);
	// 	}else{
	// 		$fis=array($fpis_neg, $fnis_neg);
	// 	}
	...
	// 	$di_neg = array();
	// 	$i = 0; $j = 0; $x =0; $y = 0;
	// 	$fis_pos = $this->getFis('positive');
	// 	$fis_neg = $this->getFis('negative');
	// 	$sum_pos = 0;
	// 	$sum_neg = 0;
	// 	$sum_arr_pos = array();
	// 	$sum_arr_neg = array();
	...
	// 	if($types[$j]=='positive'){ //positive criteria
	// 		while($i<sizeof($w_fdm)){ 
	// 			$alt = array($w_fdm[$i][$j][0],$w_fdm[$i][$j][1],$w_fdm[$i][$j][2]);	
	// 			$di_pos[$i][$j] = abs(array_sum($fis_pos[0])-array_sum($alt));
	// 			$di_neg[$i][$j] = abs(array_sum($fis_pos[1])-array_sum($alt));
	// 			$i++;
	// 		}
	// 	}else{ //negative criteria
	// 		while($i<sizeof($w_fdm)){ 
	// 			$alt = array($w_fdm[$i][$j][0],$w_fdm[$i][$j][1],$w_fdm[$i][$j][2]);	
	// 			$di_pos[$i][$j] = abs(array_sum($fis_neg[0])-array_sum($alt));
	// 			$di_neg[$i][$j] = abs(array_sum($fis_neg[1])-array_sum($alt));
	// 			// echo array_sum($fis_neg[1]); echo " - "; echo array_sum($alt); echo " = "; echo $di_neg[$i][$j];
	// 			// echo "<br>";
	// 			$i++;
	// 		}			
	// 	}
